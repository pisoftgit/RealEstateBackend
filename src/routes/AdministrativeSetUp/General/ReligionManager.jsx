...
import useReligionActions from "../../../hooks/General/useReligion";
...
const ReligionManager = () => {
  const { religions, loading, addReligion, updateReligion, deleteReligion } = useReligionActions();
  const [newReligion, setNewReligion] = useState("");
  const [editingId, setEditingId] = useState(null);

  // This will be used to store the data for the religion being edited
  const [updatedReligion, setUpdatedReligion] = useState("");

  useEffect(() => {
    if (editingId !== null) {
      const religionToEdit = religions.find((religion) => religion.id === editingId);
      setUpdatedReligion(religionToEdit?.name || "");
    }
  }, [editingId, religions]);

  const handleAddReligion = async (e) => {
    e.preventDefault();
    if (newReligion) {
      await addReligion(newReligion);
      setNewReligion("");
    }
  };
  ...
  const handleUpdateReligion = async (e) => {
    e.preventDefault();
    if (updatedReligion && editingId !== null) {
      await updateReligion(editingId, updatedReligion);
      setEditingId(null); // Reset the edit mode
      setUpdatedReligion(""); // Clear the update input
    }
  };

  const handleEditReligion = (id) => {
    setEditingId(id);
  };
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <h2 className="text-2xl font-bold mb-6 text-blue-950 dark:text-white border-b pb-2">
            {editingId ? "Update Religion" : "Add New Religion"}
          </h2>

          <form
            onSubmit={editingId ? handleUpdateReligion : handleAddReligion}
            className="space-y-5"
          >
            <div>
              ...
              <input
                type="text"
                value={editingId ? updatedReligion : newReligion}
                onChange={(e) =>
                  editingId
                    ? setUpdatedReligion(e.target.value)
        ...
        >
          <h2 className="text-2xl font-bold mb-6 border-b pb-2 text-blue-950 dark:text-white">
            Existing Religions ({religions.length})
          </h2>
            ...
            <AnimatePresence>
              <div className="space-y-3">
                {religions.map((religion) => (
                  ...
                  >
                    <div>
                      <div className="text-lg font-semibold">{religion.name}</div>
                    </div>

                    <div className="flex gap-2">
                      <button
                        onClick={() => handleEditReligion(religion.id)}
                      ...
                      <button
                        onClick={() => deleteReligion(religion.id)}
