...
const UpdateProject = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        // Preload media files
        if (data.mediaDTOs?.length > 0) {
          setMediaFiles(
            data.mediaDTOs.map((m) => ({
              id: m.id,
              mediaLabel: m.mediaLabel || "",
              file: null,
              contentType: m.contentType || "image/png",
              mediaBase64: null, // will be updated if user changes
              isExisting: true, // mark existing media
            }))
          );
        }
    ...
    const fetchPossessionStatus = async () => {
      try {
        ...
        const statusOptions = (res.data || []).map((s) => ({
          value: s,
          label: s === "READY_TO_MOVE" ? "Ready to Move" : s === "UNDER_CONSTRUCTION" ? "Under Construction" : s,
        }));
    ...
    if (field === "file" && value) {
      ...
      reader.onloadend = () => {
        setMediaFiles((prev) =>
          prev.map((m) =>
            m.id === id
              ? { ...m, file: value, mediaBase64: reader.result.split(",")[1], isExisting: false }
              : m
          )
        );
      };
  ...
  const addMediaFile = () =>
    setMediaFiles((prev) => [...prev, { id: Date.now(), mediaLabel: "", file: null, contentType: "image/png", isExisting: false }]);

  // Remove media file
  const removeMediaFile = async (id) => {
    const mediaToDelete = mediaFiles.find((m) => m.id === id && m.isExisting);
    if (mediaToDelete) {
      try {
        const secretKey = localStorage.getItem("authToken");
        await axios.delete(`${backendUrl}/property-media/deletePropertyMediaById/${id}`, {
          headers: { secret_key: secretKey },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const secretKey = localStorage.getItem("authToken");
      await axios.put(`${backendUrl}/real-estate-projects/updateProject/${projectId}`, payload, {
        headers: { secret_key: secretKey },
      });
  ...
  const inputClasses = "mt-1 w-full border p-3 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white";
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 p-2 lg:p-10 font-dm">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-10 rounded-xl shadow-lg space-y-8">
        {/* Builder */}
        <div>
          <label className={labelClasses}>Builder *</label>
          <select name="builderId" value={form.builderId} onChange={handleFormChange} className={inputClasses} required>
        ...
        <div>
          ...
          <input type="text" name="projectName" value={form.projectName} onChange={handleFormChange} className={inputClasses} required />
        </div>

        {/* RERA */}
        <div className="flex items-center gap-3">
          <input type="checkbox" name="isReraApproved" checked={form.isReraApproved} onChange={handleFormChange} />
        ...
        {form.isReraApproved && (
          <>
            <div>
              ...
              <select name="rera" value={form.rera || ""} onChange={handleFormChange} className={inputClasses} required>
            ...
            <div>
              ...
              <input type="text" name="reraNo" value={form.reraNo || ""} onChange={handleFormChange} className={inputClasses} required />
            </div>
          </>
        )}
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input type="date" name="startDate" value={form.startDate} onChange={handleFormChange} className={inputClasses} required />
          </div>
          <div>
            <label className={labelClasses}>Completion Date</label>
            <input type="date" name="completionDate" value={form.completionDate} onChange={handleFormChange} className={inputClasses} required />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input type="number" name="area" value={form.area} onChange={handleFormChange} className={inputClasses} required />
          </div>
          <div>
            <label className={labelClasses}>Area Unit</label>
            <select name="areaUnitId" value={form.areaUnitId} onChange={handleFormChange} className={inputClasses} required>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div>
            ...
            <select name="country" value={form.country} onChange={handleFormChange} className={inputClasses} required>
          ...
          <div>
            ...
            <select name="state" value={form.state} onChange={handleFormChange} className={inputClasses} required>
          ...
          <div>
            ...
            <select name="district" value={form.district} onChange={handleFormChange} className={inputClasses} required>
        ...
        <div>
          ...
          <input type="text" name="city" value={form.city} onChange={handleFormChange} className={inputClasses} />
        </div>
        <div>
          <label className={labelClasses}>Address Line 1</label>
          <input type="text" name="address1" value={form.address1} onChange={handleFormChange} className={inputClasses} />
        </div>
        <div>
          <label className={labelClasses}>Address Line 2</label>
          <input type="text" name="address2" value={form.address2} onChange={handleFormChange} className={inputClasses} />
        </div>
        <div>
          <label className={labelClasses}>Pincode</label>
          <input type="text" name="pincode" value={form.pincode} onChange={handleFormChange} className={inputClasses} />
        </div>

        {/* Possession Status */}
        <div>
          <label className={labelClasses}>Possession Status</label>
          <select name="possessionStatus" value={form.possessionStatus} onChange={handleFormChange} className={inputClasses} required>
          ...
          <div className="flex flex-wrap gap-4">
            {plcs.map((p) => (
              <label key={p.id} className="flex items-center gap-2">
                <input type="checkbox" checked={form.plc.includes(p.id)} onChange={() => handlePlcChange(p.id)} />
                {p.name}
              </label>
            ))}
          </div>
        ...
        <div>
          ...
          <textarea name="description" value={form.description} onChange={handleFormChange} className={inputClasses} />
        </div>
