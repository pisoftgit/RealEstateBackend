...
const useEmployeeTypesActions = () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setEmployeeTypes(data.map(item => ({
          id: item.id.toString(),
          employeeType: item.employeeType || "N/A",
          employeeCodeGenerate: item.employeeCodeGenerate || false,
        })));
      } else {
        setEmployeeTypes([]);
      }
  ...
  const addEmployeeType = useCallback(
    async ({ employeeType, employeeCodeGenerate }) => {
      try {
        const secretKey = localStorage.getItem("authToken");
        if (!secretKey) throw new Error("Missing authentication token");
  ...
  const updateEmployeeType = useCallback(
    async (id, { employeeType, employeeCodeGenerate }) => {
      try {
        const secretKey = localStorage.getItem("authToken");
        if (!secretKey) throw new Error("Missing authentication token");
...
export default useEmployeeTypesActions;
